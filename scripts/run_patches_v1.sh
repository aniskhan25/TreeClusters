...
if [ ! -f "${SPLIT_PREFIX}00.csv" ]; then
    ...
    echo "[INFO] Splitting tree_coordinates.csv into batches..."
    tail -n +2 "$TREECLUST_OUTPUT_PATH/tree_coordinates.csv" | split -l $LINES_PER_BATCH -d --additional-suffix=.csv - "$SPLIT_PREFIX"
